...
/**
 * Accounting/ERP export (PRD FR-31): invoices as CSV, not a native ledger or
 * a specific ERP's own API — "integration-ready", the PRD's own words.
 */
...
export async function GET() {
  ...
  const { data: invoices } = await supabase
    ...
    .select("invoice_number, status, subtotal_cents, tax_cents, total_cents, currency, payment_terms, due_date, submitted_at, approved_at, paid_at, payment_reference, edospmis_cases(case_number), edospmis_purchase_orders(po_number), edospmis_suppliers(name)")
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="${session.tenant.name.replace(/[^a-z0-9]+/gi, "-")}-invoices.csv"`,
    },
  });
}
